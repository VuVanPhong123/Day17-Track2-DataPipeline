...
{% macro normalize_priority(col) %}
    case
        when trim(cast({{ col }} as varchar)) in ('1', '2', '3', '4')
...
{% macro priority_reject_reason(col) %}
    case
        ...
        when trim(cast({{ col }} as varchar)) = '' then 'priority rỗng'
        when try_cast({{ col }} as integer) is not null
             and try_cast({{ col }} as integer) not between 1 and 4
            then 'priority là số ngoài miền 1..4'
        else 'priority không nhận diện được'
    end
{% endmacro %}
